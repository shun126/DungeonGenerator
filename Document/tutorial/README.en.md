...
The diagram below shows the minimum assets you usually create first, and where they are assigned.

```mermaid
graph TD;
    Level["Level"] --> GenerateActor["ADungeonGenerateActor"]
    GenerateActor --> Parameter["Generate parameter<br/>UDungeonGenerateParameter"]
    Parameter --> RoomMeshDb["Room Mesh Set Database"]
    Parameter --> AisleMeshDb["Aisle Mesh Set Database"]
    Parameter --> InteriorDb["Interior Database<br/>optional"]
    Parameter --> SensorDb["Gameplay Room Sensor<br/>optional"]
    Parameter --> SubLevelDb["SubLevel Database<br/>optional"]
    RoomMeshDb --> RoomVisuals["Room floors, walls, roofs, chandeliers"]
    AisleMeshDb --> AisleVisuals["Aisle floors, walls, roofs"]
    InteriorDb --> Decoration["Furniture, decoration, vegetation"]
    SensorDb --> Gameplay["Room-entry gameplay events"]
    SubLevelDb --> SpecialRooms["Hand-authored start, goal, or special rooms"]
```
...
## Advanced
- [LoadReduction.en.md](./LoadReduction.en.md)
  Keep large runtime dungeons comfortable by using `ADungeonMainLevelScriptActor` and `UDungeonComponentActivatorComponent` to reduce work for far-away actors, lights, AI, collision, and Tick.
...
- The sublevel `GridSize` does not match `Theme.HorizontalGridSize` / `Theme.VerticalGridSize` in `UDungeonGenerateParameter`
...
- [LoadReduction.en.md](./LoadReduction.en.md)
  Read this next if your runtime dungeon contains many enemies, lights, decorations, or collision-heavy actors.
