...
各テクスチャは、選択した階層までを重ねた累積表示です。下層は暗い床と細い壁で残り、選択中の階層は明るい床と太い壁で表示されます。1つの階層には、その階層の基準高さから次の階層の基準高さ未満までの、高さ帯全体が含まれます。最上階はダンジョンのボクセル領域上端まで含まれます。これにより、背の高い部屋、階段、スロープが所属する階層から欠けず、最上階も上端に近づくほど薄くなることはありません。

生成された階層の基準高さは、ミニマップが切り替わる正確な境界でもあります。Characterはコリジョンカプセルの底面がこの高さを越えた時に階層が切り替わり、それ以外のPawnはActor原点を使用します。Dungeon ActorのワールドZがボクセル原点となるため、Dungeon Actorを上下へ移動すると、デバッグ階層面とミニマップ境界も同じ量だけ移動します。
...
## 補足
実際の組み込み例は、同梱の `Content/Maps/Demonstration.umap` を開き、プラグインコンテンツ内の Widget Blueprint `Content/Widget/WBP_SampleDungeonPlayGame` を参照してください。
