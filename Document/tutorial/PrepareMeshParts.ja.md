...
最初は細かいバリエーションを増やす必要はありません。まず必須の **床 / 壁 / 天井** を用意し、高低差を使う前に **スロープ** を追加します。正しい向きと原点で登録できる状態を目指します。
...
## 最初に用意するもの
平坦なダンジョンを最初に確認する場合、必須なのは次の 3 種類です。

- `Floor Parts`
- `Wall Parts`
- `Roof Parts`
- `Slope Parts` は空でも検証エラーにはなりませんが、生成された高低差を正しく表示するために必要です。上下移動を使う前に追加してください。

これらを用意すると床・壁・天井を表示でき、スロープを追加すると段差も表示できます。
...
### 作るときのルール
- **Z 軸方向が上面** になるようにします
- **原点は底面中央** にし、配置面からメッシュの厚みが上へ伸びるようにします
...
### 作るときのルール
- 天井を **XY 平面** に置き、**Z 軸が上**を向くようにします
- **原点は天面中央** にし、メッシュの厚みが天井から下へ伸びるようにします
- 模様に方向がある場合は制作時の正面を統一し、パーツの `PlacementDirection` または Relative Transform で回転を調整します
- 物理オブジェクトの抜けを防ぐため、**ある程度の厚み** を付けます
...
- 見た目の角度は、水平グリッドサイズと垂直グリッドサイズの比率で決まります

### 作るときのルール
- **Y 軸方向が正面** になるようにします
- **原点は 2 グリッド分を含むメッシュ全体の底面中央** にします
- 物理オブジェクトの抜けを防ぐため、**ある程度の厚み** を付けます
...
- 原点がメッシュ全体の中央ではなく端に寄っていて、床とのつながりがずれる
...
## よくある失敗
- 必須の床 / 壁 / 天井のどれかが未登録で、検証に失敗する
- 高低差のあるレイアウトでスロープがなく、段差部分が正しく見えない
