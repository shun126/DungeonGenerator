...
#include "DungeonPointLightComponent.generated.h"

/**
 * Dungeon point light component with reusable fade and volume control.
 * 再利用可能なフェードとボリューム制御を持つ Dungeon 用 Point Light コンポーネントです。
 */
UCLASS(ClassGroup = "DungeonGenerator", meta = (BlueprintSpawnableComponent))
class DUNGEONGENERATOR_API UDungeonPointLightComponent : public UPointLightComponent
{
	GENERATED_BODY()

public:
	/**
	 * Creates a Dungeon point light with fade ticking disabled until needed.
	 * 必要になるまでフェード Tick を無効にした Dungeon Point Light を作成します。
	 */
	explicit UDungeonPointLightComponent(const FObjectInitializer& objectInitializer);
	virtual ~UDungeonPointLightComponent() override = default;
