...
public:
	/*
	 * Destroys the manager and clears pending requests without callbacks.
	...
	/*
	 * Sets the maximum number of actors spawned per update.
	 * 1回の更新で生成するActor数の上限を設定します。
	 */
	void SetMaxSpawnsPerUpdate(int32 InMaxSpawnsPerUpdate);

	/*
	 * Sets the maximum time in seconds spent per update.
	...
	/*
	 * Adds a deferred actor spawn request.
	 * 遅延Actor生成要求を追加します。
	 */
	void RequestSpawn(
		UWorld* World,
...
private:
	...
	/*
	 * Maximum actors spawned per update. 0 or less means unlimited.
	 * 更新ごとの最大Actor生成数です。0以下は無制限です。
	 */
