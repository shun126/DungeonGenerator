...
struct DUNGEONGENERATOR_API FDungeonMigrationMessage
{
	 ...
	 * この移行メッセージの重要度です。
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "DungeonGenerator|Migration")
...
public:
	/*
	 * Returns the serialized asset format version captured during loading.
	...
	/*
	 * Returns mutable editor-only migration state for this asset.
	 * このアセットの変更可能なEditor専用移行状態を返します。
	 */
	virtual FDungeonAssetMigrationState& GetMutableDungeonAssetMigrationState() = 0;

	/*
	 * Returns editor-only migration state for this asset.
