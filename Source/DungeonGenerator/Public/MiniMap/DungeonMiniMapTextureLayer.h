...
private:
	...
	*/
	...
	/**
	Gets the length of a voxel edge.
	*/
	float GetGridScale() const noexcept;

	/**
	Get scale value from world space to voxel space
	*/
	float GetGeneratedScale() const noexcept;

	/**
	Get scale value from world space to texture space
	*/
	float GetScaleWorldToTexture() const noexcept;

	const std::vector<int32_t>& GetFloorElevations() const noexcept;
...
inline float UDungeonMiniMapTextureLayer::GetGridScale() const noexcept
{
	return mGridSize;
}
