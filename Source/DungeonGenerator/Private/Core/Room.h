...
namespace dungeon
{
	...
	*/
	...
	public:
		...
		コンストラクタ
		*/
		explicit Room(const GenerateParameter& parameter);
		...
		*/
		...
		\return		床下の高さ
		*/
		uint16_t GetUnderfloorHeight() const;
		...
		*/
		void SetUnderfloorHeight(const uint16_t underfloorHeight);
		...
		*/
		...
		矩形を取得します
		*/
		FIntRect GetRect() const;

		/*!
		立方体の手前の座標（軸の大きい方）を取得します
		*/
		int32_t GetForeground() const;

		/*!
		立方体の奥の座標（軸の小さい方）を取得します
		*/
		int32_t GetBackground() const;
		...
		*/
		...
		\return		床の中心
		*/
		Point GetFloorCenter() const;

		/*!
		地面の中心の座標を取得します
		\return		地面の中心
		*/
		Point GetGroundCenter() const;
		...
		*/
		bool Intersect(const Room& other, const uint32_t margin) const;
		...
		*/
		...
		\param[in]		z		調べる点
		*/
		bool Contain(const int32_t x, const int32_t y, const int32_t z) const;
		...
		*/
		uint8_t GetDepthFromStart() const;

		/*!
		スタート位置からの深さを取得します
		\param[in]	depthFrcomStart		スタート位置からの深さ（部屋の数）
		*/
		void SetDepthFromStart(const uint8_t depthFrcomStart);


		uint8_t GetBranchId() const;
		void SetBranchId(const uint8_t branchId);
