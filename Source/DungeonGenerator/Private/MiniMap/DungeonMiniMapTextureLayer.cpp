...
#include "MiniMap/DungeonMiniMapTexture.h"
...
float UDungeonMiniMapTextureLayer::GetGeneratedScale() const noexcept
{
	return (DungeonMiniMapTextures.Num() > 0) ? DungeonMiniMapTextures[0]->GetGeneratedScale() : 1.f;
}

float UDungeonMiniMapTextureLayer::GetScaleWorldToTexture() const noexcept
{
	return 1.f / mGridSize * GetGeneratedScale();
}

const std::vector<int32_t>& UDungeonMiniMapTextureLayer::GetFloorElevations() const noexcept
{
	return mGenerator->GetFloorHeight();
}

FVector2D UDungeonMiniMapTextureLayer::ToRelative(const FVector& location) const
{
	const float ratio = GetScaleWorldToTexture();
