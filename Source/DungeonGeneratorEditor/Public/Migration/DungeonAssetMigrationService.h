...
private:
	...
	/*
	 * Returns all migratable DungeonGenerator assets contained in a package.
	...
	/*
	 * Adds a migration message to a state object.
	 * 移行状態へ移行メッセージを追加します。
	 */
	...
	/*
	 * Returns true when the state contains at least one error.
	 * 状態にErrorが1件以上含まれる場合にtrueを返します。
	 */
	static bool HasErrors(const FDungeonAssetMigrationState& State);

	/*
	 * Returns true when the state contains at least one warning.
